'''
改自火山引擎语音技术流失语音识别demo  https://www.volcengine.com/docs/6561/80818
修改后可以处理客户端发来的实时音频流，并进行处理再返回客户端信息
'''
    ...
    async def data_processor(self, websocket):
        ...
        async with websockets.connect(self.ws_url, extra_headers=header, max_size=1000000000) as ws:
            ...
            while True:
                ...
                # # 如果想要读取wav文件数据，但识别不了可以尝试将这段代码打开
                # tmp_f = BytesIO()
                # out_wf = wave.open(tmp_f, "wb")
                # out_wf.setnchannels(2)  # 声道数
                # out_wf.setsampwidth(2)  # 采样宽度
                # out_wf.setframerate(16000)  # 采样率
                # out_wf.writeframes(audio_segment)
                # audio_segment = tmp_f.getvalue()
